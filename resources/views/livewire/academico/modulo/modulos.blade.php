<div>
    <div class="bg-blue-200 rounded-lg align-middle p-2 mb-2 text-center">
        <h1 class="text-xl uppercase">modulos</h1>
    </div>
        ...
        <div class="relative overflow-x-auto">
            <table class=" text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th scope="col" class="px-6 py-3" style="modulor: pointer;" wire:click="organizar('id')">
                            ID
                            @if ($ordena != 'id')
                        ...
                        <th scope="col" class="px-6 py-3" style="modulor: pointer;" wire:click="organizar('slug')">
                            Código modulo
                            @if ($ordena != 'slug')
                                <i class="fas fa-sort"></i>
                            @else
                        ...
                        <th scope="col" class="px-6 py-3" style="modulor: pointer;" wire:click="organizar('name')">
                            modulo
                            @if ($ordena != 'name')
                                <i class="fas fa-sort"></i>
                            @else
                        ...
                        <th scope="col" class="px-6 py-3" style="modulor: pointer;" wire:click="organizar('created_at')">
                            Fecha de Creación
                    ...
                    @foreach ($modulos as $modulo)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-green-200">
                            ...
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                @can('ac_moduloEditar')
                                    @if ($modulo->status===1)
                                        <a href="#" wire:click.prevent="show({{$modulo}},{{0}})" class="text-black bg-gradient-to-r from-blue-300 via-blue-400 to-blue-500 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-200 dark:focus:ring-blue-700 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 capitalize">
                                            <i class="fa-solid fa-marker"></i>
                                        </a>
                                    @endif
                                @endcan
                                @can('ac_moduloInactivar')
                                    <a href="#" wire:click.prevent="show({{$modulo}},{{1}})" class="text-black bg-gradient-to-r from-yellow-300 via-yellow-400 to-yellow-500 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-yellow-200 dark:focus:ring-yellow-700 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 capitalize">
            ...
            <div class="mt-2 p-1 w-auto rounded-lg grid grid-cols-2 gap-4 bg-blue-100">
                <div>
                    <label class="relative inline-flex items-center mb-4 modulor-pointer">
    ...
    @if ($is_creating)
        <livewire:academico.modulo.modulos-crear />
    @endif

    @if ($is_editing)
        <livewire:academico.modulo.modulos-editar :elegido="$elegido" />
    @endif

    @if ($is_deleting)
        <livewire:academico.modulo.modulos-inactivar :elegido="$elegido" />
    @endif
